...
output_gpkg_path = r"outputs\korea_integrated_dataset_v4_jeju_mw.gpkg"
...
filtered_energy_facilities_df.loc[:, 'Type'] = filtered_energy_facilities_df['Type'].replace({
...
def adjust_capacity_proportionally(facilities_df):
    ...
    import importlib
    ember_analysis = importlib.import_module('p1_b_ember_analysis_mw')
    
    # Import capacity values
    target_capacities = {
        "Fossil": ember_analysis.larger_value_fossil,
        ...
        "Other Renewables": ember_analysis.larger_value_other_renewables,
    }
...
def calculate_centroid_mw(centroids_gdf, facilities_gdf):
    # Calculate total available MW
    total_available_mw = facilities_gdf['Adjusted_Capacity_MW'].sum()
    ...
    centroids_gdf['needed_mw'] = centroids_gdf['proportion'] * total_available_mw
    
    return centroids_gdf

# Apply the calculation
population_centroids_gdf = calculate_centroid_mw(population_centroids_gdf, energy_facilities)
...
print("\n<Before Processing>")
total_available_mw = energy_facilities_proj['Adjusted_Capacity_MW'].sum()
total_population = population_centroids_proj['population'].sum()
num_centroids = len(population_centroids_proj)
print(f"1. Total available MW from Facilities: {total_available_mw:,.2f}")
...
def process_energy_supply(shortest_paths_df, facilities_df, centroids_df):
    # Create a copy of facilities DataFrame to track remaining MW
    facilities_remaining = facilities_df.copy()
    # If 'Remaining_MW' already exists, use it as the starting point
    if 'Remaining_MW' in facilities_df.columns:
        facilities_remaining['Remaining_MW'] = facilities_df['Remaining_MW']
    else:
        facilities_remaining['Remaining_MW'] = facilities_remaining['Adjusted_Capacity_MW']
    ...
    supply_status = gpd.GeoDataFrame({
        ...
        'needed_mw': centroids_df['needed_mw'],
        'filled_mw': 0.0,
    ...
    for _, path in sorted_paths.iterrows():
        ...
        # Get needed and available MW
        needed_mw = float(supply_status.loc[centroid_id, 'needed_mw'])
        facility_mask = facilities_remaining['GEM unit/phase ID'] == facility_id
        
        if not any(facility_mask):
            continue
            
        available_mw = float(facilities_remaining.loc[facility_mask, 'Remaining_MW'].iloc[0])
        ...
        if available_mw >= needed_mw:
            ...
            supply_status.loc[centroid_id, 'filled_mw'] = needed_mw
            supply_status.loc[centroid_id, 'supply_status'] = 'filled'
            facilities_remaining.loc[facility_mask, 'Remaining_MW'] -= needed_mw
            ...
            supply_status.loc[centroid_id, 'filled_mw'] = available_mw
            supply_status.loc[centroid_id, 'supply_status'] = 'partially_filled'
            facilities_remaining.loc[facility_mask, 'Remaining_MW'] = 0
            supply_status.loc[centroid_id, 'facility_id'] = facility_id
            supply_status.loc[centroid_id, 'facility_type'] = facilities_df.loc[facility_mask, 'Type'].iloc[0]  # Add this line
...
energy_facilities_proj['Supplied_MW'] = energy_facilities_proj.apply(
    lambda x: energy_supply_status[
        energy_supply_status['facility_id'] == x['GEM unit/phase ID']
    ]['filled_mw'].sum(),
    axis=1
)
energy_facilities_proj['Remaining_MW'] = energy_facilities_proj['Adjusted_Capacity_MW'] - energy_facilities_proj['Supplied_MW']

# Now print the statistics
print("\n<After first round energy supply processing>")
total_supplied_mw = energy_facilities_proj['Supplied_MW'].sum()
total_remaining_mw = energy_facilities_proj['Remaining_MW'].sum()
...
print(f"1. Total Supplied MW: {total_supplied_mw:,.2f}")
print(f"   Remaining MW: {total_remaining_mw:,.2f}")
...
for _, facility in energy_facilities_proj.iterrows():
    ...
    annual_mw = facility['Adjusted_Capacity_MW']
    supplied_mw = facility['Supplied_MW']
    remaining_mw = facility['Remaining_MW']
    print(f"Facility {facility_id}:")
    print(f"  Adjusted Capacity MW: {annual_mw:,.2f}")
    print(f"  Supplied MW: {supplied_mw:,.2f}")
    print(f"  Remaining MW: {remaining_mw:,.2f}")
...
remaining_facilities = facilities_remaining[facilities_remaining['Remaining_MW'] > 0]
...
# Get the filled_mw from first round energy supply status
first_round_filled = energy_supply_status[['centroid_id', 'needed_mw', 'filled_mw']].copy()
first_round_filled.rename(columns={'needed_mw': 'needed_mw_first_round'}, inplace=True)

# Update unfilled_centroids_proj with remaining needed_mw
...
unfilled_centroids_proj['needed_mw'] = unfilled_centroids_proj['needed_mw_first_round'] - unfilled_centroids_proj['filled_mw']
...
# Rename needed_mw in second_energy_supply_status for clarity
second_energy_supply_status.rename(columns={'needed_mw': 'needed_mw_after_first_processing'}, inplace=True)
...
# After second round process_energy_supply, calculate Second_supplied_MW and Second_remaining_MW
energy_facilities_proj['Second_supplied_MW'] = energy_facilities_proj.apply(
    lambda x: second_energy_supply_status[
        second_energy_supply_status['facility_id'] == x['GEM unit/phase ID']
    ]['filled_mw'].sum(),
    axis=1
)
energy_facilities_proj['Second_remaining_MW'] = energy_facilities_proj['Remaining_MW'] - energy_facilities_proj['Second_supplied_MW']

print("\n<After second round energy supply processing>")
total_supplied_mw_second = energy_facilities_proj['Second_supplied_MW'].sum()
total_remaining_mw_final = energy_facilities_proj['Second_remaining_MW'].sum()
...
print(f"1. Second Round Supplied MW: {total_supplied_mw_second:,.2f}")
print(f"   Final Remaining MW: {total_remaining_mw_final:,.2f}")
...
for _, facility in energy_facilities_proj.iterrows():
    facility_id = facility['GEM unit/phase ID']
    annual_mw = facility['Adjusted_Capacity_MW']
    first_supplied = facility['Supplied_MW']
    remaining_first = facility['Remaining_MW']
    second_supplied = facility['Second_supplied_MW']
    final_remaining = facility['Second_remaining_MW']
    print(f"Facility {facility_id}:")
    print(f"  Adjusted Capacity MW: {annual_mw:,.2f}")
