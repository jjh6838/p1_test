...
hazard_types = ['flooding', 'cyclones', 'wildfires', 'landslides', 'droughts', 'extreme_heat']
...
for energy_type in energy_types:
    for year in years:
        ...
        for hazard_type in hazard_types:
            ...
            hazard_factor = np.random.uniform(0.9, 1.1)
            ...
            for supply_idx, supply in enumerate(supply_scenarios):
                ...
                for buffer_idx, buffer in enumerate(hazard_buffers):
                    ...
                    key = f"{energy_type}_{year}_{hazard_type}_supply_{supply}_{buffer}"
                    
                    # Exposure: affected by hazard type, supply and buffer
                    # Add small random variation (±2%) for realism
                    variation = np.random.uniform(0.98, 1.02)
                    exposure_percentages[key] = base_exp * hazard_factor * supply_factor * buffer_factor * variation
                    
                    # Risk-avoidance: affected by hazard type and supply but CONSTANT across buffers
                    # Only small random variation (±1%) for realism
                    risk_variation = np.random.uniform(0.99, 1.01)
                    risk_avoid_percentages[key] = base_risk * hazard_factor * supply_factor * risk_variation
...
total_combinations = len(energy_types) * len(years) * len(hazard_types) * len(supply_scenarios) * len(hazard_buffers)
            ...
            for hazard_type in hazard_types:
                for supply in supply_scenarios:
                    for buffer in hazard_buffers:
                        ...
                        long_data.append({
                            'ISO3_code': iso3,
                            'energy_type': energy_type.replace('_', ' ').title(),
                            'year': year,
...
print("  - energy_type: Hydro, Solar, Wind, Other Renewables, Nuclear, Fossil")
print("  - year: 2030, 2050")
print("  - hazard_type: Flooding, Cyclones, Wildfires, Landslides, Droughts, Extreme Heat")
