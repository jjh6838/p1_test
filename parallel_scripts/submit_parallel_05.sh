...
echo "[INFO] Starting parallel script 5/40 (T3) at $(date)"
...
if $PY process_country_supply.py AUS --output-dir outputs_per_country; then
    echo "[SUCCESS] AUS completed"
else
    echo "[ERROR] AUS failed"
fi

echo "[INFO] Batch 5/40 (T3) completed at $(date)"
