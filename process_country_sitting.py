...
"""
# ==================================================================================================
# SCRIPT PURPOSE: Siting Analysis for Remote Settlement Electrification
# ==================================================================================================
#
# WHAT THIS CODE DOES:
...
#
# KEY STEPS:
# 1. Filter Settlements: Identifies settlements that are not fully served (excludes 'Filled' 
#    settlements) and removes the bottom 1% by demand to focus on significant demand centers.
# 2. Cluster Analysis: Groups nearby underserved settlements using DBSCAN spatial clustering
#    (100km radius) to identify regional demand centers.
# 3. Grid Proximity: Calculates distances from cluster centers to nearest existing grid lines,
#    identifying remote clusters beyond 50km threshold.
# 4. Network Design: For remote clusters, designs minimum spanning tree (MST) style networks
#    (Steiner-like) to connect settlements efficiently.
#
# OUTPUTS:
# - Parquet files with cluster assignments and grid connection analysis
# - Summary statistics on remote settlement electrification needs
# ==================================================================================================
"""
...
ANALYSIS_YEAR = 2030
SUPPLY_FACTOR = 1.0
CLUSTER_RADIUS_KM = 100  # Cluster settlements within 100km
GRID_DISTANCE_THRESHOLD_KM = 50  # Consider clusters beyond 50km as "remote"
DROP_PERCENTAGE = 0.01  # Drop bottom 1% of partially filled settlements
...
def filter_settlements(centroids_gdf):
    """
    Step 1: Filter settlements by removing 'Filled' and dropping bottom 1% by demand.
    
    Returns settlements that need grid connection consideration.
    """
    print("\n" + "="*60)
    print("STEP 1: FILTER SETTLEMENTS")
    print("="*60)
    
    # Count initial settlements by status
    status_counts = centroids_gdf['supply_status'].value_counts()
    print(f"\nInitial settlement status:")
    for status, count in status_counts.items():
        print(f"  {status}: {count:,}")
    
    # Exclude 'Filled' settlements
    unfilled = centroids_gdf[centroids_gdf['supply_status'] != 'Filled'].copy()
    print(f"\nAfter removing 'Filled': {len(unfilled):,} settlements")
    
    # Filter out 'No Demand' settlements
    unfilled = unfilled[unfilled['supply_status'] != 'No Demand'].copy()
    print(f"After removing 'No Demand': {len(unfilled):,} settlements")
    
    if len(unfilled) == 0:
        print("No unfilled settlements found!")
        return unfilled
    
    # Drop bottom 1% by supply_received_mwh
    if len(unfilled) >= 100:
        drop_count = max(1, int(len(unfilled) * DROP_PERCENTAGE))
        unfilled_sorted = unfilled.sort_values('supply_received_mwh', ascending=True)
        threshold_mwh = unfilled_sorted.iloc[drop_count - 1]['supply_received_mwh']
        filtered = unfilled[unfilled['supply_received_mwh'] > threshold_mwh].copy()
        print(f"\nDropping bottom {DROP_PERCENTAGE*100}% ({drop_count:,} settlements)")
        print(f"Threshold: {threshold_mwh:.2f} MWh")
        print(f"Remaining settlements: {len(filtered):,}")
    else:
        # Drop only the lowest settlement
        if len(unfilled) > 1:
            unfilled_sorted = unfilled.sort_values('supply_received_mwh', ascending=True)
            filtered = unfilled_sorted.iloc[1:].copy()
            dropped_mwh = unfilled_sorted.iloc[0]['supply_received_mwh']
            print(f"\nFewer than 100 settlements: dropping lowest settlement")
            print(f"Dropped settlement MWh: {dropped_mwh:.2f}")
            print(f"Remaining settlements: {len(filtered):,}")
        else:
            filtered = unfilled.copy()
            print(f"\nOnly 1 settlement, keeping it")
    
    return filtered


def cluster_settlements(settlements_gdf):
    """
    Step 2: Cluster unfilled settlements using DBSCAN based on spatial proximity.
    
    Returns settlements with cluster assignments.
    """
    print("\n" + "="*60)
    ...
    # Convert cluster radius from km to approximate degrees
    # At equator: 1 degree ≈ 111 km
    # Use conservative estimate considering latitude variations
    eps_degrees = CLUSTER_RADIUS_KM / 111.0
    
    print(f"\nClustering parameters:")
    print(f"  Radius: {CLUSTER_RADIUS_KM} km (~{eps_degrees:.4f} degrees)")
    print(f"  Min samples: 1 (all points assigned to clusters)")
    
    # Perform DBSCAN clustering
    clustering = DBSCAN(eps=eps_degrees, min_samples=1, metric='haversine')
...
def calculate_cluster_centers(settlements_gdf):
    """Calculate cluster center points (centroid of settlements in each cluster)."""
    cluster_centers = []
    ...
    for cluster_id in settlements_gdf['cluster_id'].unique():
        ...
        # Calculate weighted centroid by demand
        ...
        if demand_col in cluster_points.columns:
            ...
            # Use supply_received_mwh as proxy if demand column not available
    ...
    if not cluster_centers:
        return gpd.GeoDataFrame(columns=['cluster_id', 'geometry', 'num_settlements', 
                                        'total_demand_mwh', 'center_lon', 'center_lat'],
                               crs=COMMON_CRS)
...
def compute_grid_distances(cluster_centers_gdf, grid_lines_gdf):
    ...
    Returns cluster centers with grid distance information.
    """
    print("\n" + "="*60)
...
def build_steiner_network(settlements_gdf, cluster_id):
    ...
    Returns network edges as LineStrings.
    """
...
def save_outputs(settlements_gdf, cluster_centers_gdf, networks_gdf, country_iso3, 
    ...
    # Save settlements with cluster assignments
    ...
    # Save cluster centers with grid distances
    ...
    # Save network edges for remote clusters
...
def process_country_siting(country_iso3, output_dir="outputs_per_country"):
    ...
    try:
        ...
        # Step 2: Cluster settlements
        settlements_gdf = cluster_settlements(settlements_gdf)
        
        # Calculate cluster centers
        cluster_centers_gdf = calculate_cluster_centers(settlements_gdf)
        
        # Step 3: Compute grid distances
        cluster_centers_gdf = compute_grid_distances(cluster_centers_gdf, grid_lines_gdf)
        
        # Step 4: Build Steiner networks for remote clusters
...
def main():
    ...
    if result:
        print(f"\nSuccessfully processed {args.country_iso3}")
        return 0
    else:
        print(f"\nFailed to process {args.country_iso3}")
        return 1
