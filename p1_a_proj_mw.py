...
def disaggregate(row, merged_df, categories, column_name):
    ...
    total_2023 = sum(merged_df.loc[merged_df["ISO3_code"] == country_code, f"{cat}_2023"].values[0] for cat in categories)
    if total_2023 == 0:
        print(f"Total 2023 for country code {country_code} is 0")
        return {cat: 0 for cat in categories}
    proportions = {cat: merged_df.loc[merged_df["ISO3_code"] == country_code, f"{cat}_2023"].values[0] / total_2023 for cat in categories}
...
save_path_final = "ember_energy_data/proj_mw.csv"
