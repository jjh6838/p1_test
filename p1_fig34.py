...
gs3 = gridspec.GridSpec(3, 4, wspace=0.25, hspace=0.3)
...
for i, group in enumerate(income_groups_sorted):
    ...
    ylim_twh = max(max_twh_exposure, max_twh_risk_avoid) * 1.1  # Add 10% padding

    # Plot TWh data - using Figure 1 colors
    ax.plot(years, income_group_exposure_twh[group], 'o-', color='#AA1A2D', linewidth=2, markersize=3, label='Exposed Generation')
    ax.plot(years, income_group_risk_avoid_twh[group], 'o-', color='#002147', linewidth=2, markersize=3, label='Exposed (With Risk-Avoidance)')
    ...
    for j, year in enumerate(years):
        ...
        if income_group_risk_avoid_twh[group][j] > 0:
...
for i, region in enumerate(regions_sorted[:4]):
    ...
    # Plot TWh data - using Figure 1 colors
    ax.plot(years, regional_group_exposure_twh[region], 'o-', color='#AA1A2D', linewidth=2, markersize=3, label='Exposed Generation')
    ax.plot(years, regional_group_risk_avoid_twh[region], 'o-', color='#002147', linewidth=2, markersize=3, label='Exposed (With Risk-Avoidance)')
...
fig3.savefig('outputs_processed_vis/fig3.pdf', bbox_inches='tight', dpi=300)


# === FIGURE 4: Custom 11 subplots (2024–2050) - Economic Value (USD) ===
fig4 = plt.figure(figsize=(7.2, 6), dpi=300)
gs4 = gridspec.GridSpec(3, 4, wspace=0.25, hspace=0.3)
...
for i, group in enumerate(income_groups_sorted_usd):
    ...
    ylim_usd = max(max_usd_exposure, max_usd_risk_avoid) * 1.1  # Add 10% padding
    ...
    for j, year in enumerate(years):
        ...
        if income_group_risk_avoid_usd[group][j] > 0:
    ...
    for j, year in enumerate(years):
        if regional_group_exposure_usd[region][j] > 0:
            ax.text(year, regional_group_exposure_usd[region][j] * 1.05, f'${regional_group_exposure_usd[region][j]:.0f}B', 
                    ha='center', va='bottom', fontsize=annot_fs, color='#E2C044')
        if regional_group_risk_avoid_usd[region][j] > 0:
...
fig4.suptitle('Figure 4. Climate Risk Exposure of Electricity Generation Value by Income Group and Region (2024–2050)', 
              fontsize=10, fontweight='bold', y=0.95)
fig4.savefig('outputs_processed_vis/fig4.pdf', bbox_inches='tight', dpi=300)
